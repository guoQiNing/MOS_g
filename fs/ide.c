...
void ide_read(u_int diskno, u_int secno, void *dst, u_int nsecs) {
	...
	for (u_int off = 0; begin + off < end; off += BY2SECT) {
		...
		panic_on(syscall_write_dev(&temp,(DEV_DISK_ADDRESS + DEV_DISK_ID),sizeof(u_int)));
		temp = begin + off;
		panic_on(syscall_write_dev(&temp,(DEV_DISK_ADDRESS + DEV_DISK_OFFSET),sizeof(u_int)));
		temp = DEV_DISK_OPERATION_READ;
		panic_on(syscall_write_dev(&temp,(DEV_DISK_ADDRESS + DEV_DISK_START_OPERATION),sizeof(u_int)));
		temp = 0;
		panic_on(syscall_read_dev(&temp,(DEV_DISK_ADDRESS + DEV_DISK_STATUS),sizeof(u_int)));
		panic_on(temp == 0);
