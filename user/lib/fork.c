...
static void __attribute__((noreturn)) cow_entry(struct Trapframe *tf) {
	...
	if (syscall_mem_map(0,(void*)UCOW,0,(void*)va,perm) < 0) {
...
static void duppage(u_int envid, u_int vpn) {
	...
	if ((perm & PTE_D) == 0 || (perm & PTE_LIBRARY) == PTE_LIBRARY || (perm & PTE_COW) == PTE_COW) {
...
int fork(void) {
	...
	if (env->env_user_tlb_mod_entry != (u_int)cow_entry) {
		try(syscall_set_tlb_mod_entry(syscall_getenvid(), cow_entry));
	}
