...
int sys_write_workdir(char* copyingBuf, int maxsize) {
	if (maxsize > MAX_PATH_SIZE) {
		return - 1;
	}
	int i;
	for (i = 0; copyingBuf[i] != 0 && i < maxsize; ++i) {
		curenv->env_work_dir[i] = copyingBuf[i];
	}
	curenv->env_work_dir[i] = 0;
	return 0;
}

int sys_read_workdir(char * copiedBuf,int maxsize) {
	int i ;
	if (strlen(curenv->env_work_dir) > maxsize) {
		return -1;
	}
	for (i = 0; curenv->env_work_dir[i] != 0 && i < MAX_PATH_SIZE; ++i) {
		copiedBuf[i] = curenv->env_work_dir[i];
	}
	copiedBuf[i] = 0;
	return 0;
}
...
void *syscall_table[MAX_SYSNO] = {
	...
	[SYS_read_workdir] = sys_read_workdir,
	[SYS_write_workdir] = sys_write_workdir,
};
...
void do_syscall(struct Trapframe *tf) {
	...
	arg4 = *((unsigned long*)(tf->regs[29] + 16));
	arg5 = *((unsigned long*)(tf->regs[29] + 20));
